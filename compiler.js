...
class TikZCompiler {
  ...
  stop(reason='Compilation interrupted.') {
  ...
  rpc(method,args,timeout=30000) {
    ...
    return new Promise((resolve,reject)=>{
      const timer=setTimeout(()=>this.stop('The diagram exceeded the 30-second compilation limit. Simplify it and try again.'),timeout);
  ...
  async load() {
    ...
    worker.onmessage=({data})=>{
      ...
      if(data.type==='uncaughtError'){this.stop(data.error?.message || 'Internal TeX engine error.');return;}
      const p=this.pending.get(data.uid);if(!p) return;
      if(data.type==='error') {
        clearTimeout(p.timer);this.pending.delete(data.uid);
        const error=new Error(data.error?.message || 'TeX engine error.');
    ...
    worker.onerror=e=>{if(this.worker===worker)this.stop(e.message || 'The TikZ engine could not start.');};
  ...
  compile(source) {
    if(typeof source !== 'string' || !source.trim()) return Promise.resolve({ok:false,error:'The TikZ code is empty.'});
    if(source.length>60000) return Promise.resolve({ok:false,error:'Limit: 60,000 characters per diagram.'});
    ...
    if(this.count>=24) return Promise.resolve({ok:false,error:'Too many diagrams in the queue. Try again shortly.'});
